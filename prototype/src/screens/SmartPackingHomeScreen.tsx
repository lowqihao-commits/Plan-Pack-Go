...
export function SmartPackingHomeScreen({ draft, mode, sampleTrip, members, suggestions, personalItems, sharedItems, delta, onBack, onSuggestions, onPersonal, onShared, onOutfits, onReminders, onUpdates }: SmartPackingHomeProps) {
    ...
    <Page className="packing-home-page" labelledBy="packing-home-title">
      ...
      <div className="packing-home-body">
        <section className="packing-hero" aria-labelledby="packing-home-title">
          <div><p className="eyebrow">{draft.name}</p><h2 id="packing-home-title">Pack from your actual plan</h2><p>Based on your itinerary and weather. Your choice.</p></div>
        ...
        {!delta.reviewed ? <button className="packing-update-card" type="button" onClick={onUpdates}><span><RefreshCw aria-hidden="true" size={19} /></span><div><strong>Review Updates</strong><small>2 packing changes from your updated itinerary.</small></div><ChevronRight aria-hidden="true" size={18} /></button> : null}
        ...
        {mode === 'group' ? <section className="group-progress-card" aria-labelledby="group-progress-title"><div className="section-heading"><h2 id="group-progress-title">Group Packing Progress</h2><span>Personal details stay private</span></div><div className="group-progress-list">{joinedMembers.map((member, index) => {
          const fallback = sampleTrip ? memberPackingProgress[member.id] || { packed: 0, total: 0 } : { packed: 0, total: 0 };
        ...
        })}</div><p>Only progress is shared. Your item names stay private.</p></section> : null}
