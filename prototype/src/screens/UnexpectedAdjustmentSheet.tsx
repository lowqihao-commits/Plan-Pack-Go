...
export function UnexpectedAdjustmentSheet({ kind, days, activeDayIndex, onClose, onApply, onResolve }: { kind: AdjustmentKind; days: ItineraryDay[]; activeDayIndex: number; onClose: () => void; onApply: (days: ItineraryDay[], message: string) => void; onResolve: (message: string) => void }) {
  ...
  const description = kind === 'unavailable' ? 'Choose what happens to this stop before the route changes.' : kind === 'late' ? `The current finish may move from ${currentSchedule.finish} to ${formatTime(currentSchedule.finishMinutes + 40)}.` : 'Only outdoor activities with a meaningful impact are shown.';
