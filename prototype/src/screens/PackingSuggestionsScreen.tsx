    ...
    <Page className="suggestions-page" labelledBy="suggestions-title">
      ...
      <div className="suggestions-body">
        ...
        {(['Needed', 'Possibly Useful'] as const).map((group) => {
          ...
          return <section className={`suggestion-group suggestion-group--${group === 'Needed' ? 'needed' : 'useful'}`} key={group} aria-labelledby={`suggestion-${group.replace(' ', '-').toLowerCase()}`}><div className="suggestion-group__heading"><h2 id={`suggestion-${group.replace(' ', '-').toLowerCase()}`}>{group}</h2><span>{groupItems.length}</span></div><p>{group === 'Needed' ? 'Essentials for your trip and activities.' : 'Optional extras for your plans.'}</p><div className="suggestion-list">{groupItems.map((item) => <SuggestionCard key={item.id} item={item} mode={mode} onDecision={onDecision} onType={onType} onQuantity={onQuantity} />)}</div></section>;
        })}
        {!active.length ? <section className="packing-complete-state"><img src="/assets/mascot-packing-complete-transparent.png" width="132" height="132" alt="" /><h2>All suggestions reviewed</h2><p>Added items are in your lists. Skipped items stay hidden.</p></section> : null}
